namespace eShop.Services.Ordering.OrderingAPI.Application.DomainEventHandlers.OrderStockConfirmed;
    ...
    public async Task Handle(OrderStatusChangedToStockConfirmedDomainEvent orderStatusChangedToStockConfirmedDomainEvent, CancellationToken cancellationToken)
    {
        _logger.CreateLogger<OrderStatusChangedToStockConfirmedDomainEventHandler>()
            .LogTrace("Order with Id: {OrderId} has been successfully updated to status {Status} ({Id})",
                orderStatusChangedToStockConfirmedDomainEvent.OrderId, nameof(OrderStatus.StockConfirmed), OrderStatus.StockConfirmed);
        ...
        await _orderingIntegrationEventService.AddAndSaveEventAsync(new
           OrderStockConfirmedIntegrationEvent(order.Id, order.OrderStatus.ToString(), buyer.Name));
