    ...
    IStateMachineActivity<OrderState, OrderSubmittedStateIntegrationEvent>
{
        ...
        )
    {
        _context = context;
        _mediator = mediator;
        _requestCheckStockForOrderIntegrationCommand = requestCheckStockForOrderIntegrationCommand;
    }
    ...
    public async Task Execute(BehaviorContext<OrderState, OrderSubmittedStateIntegrationEvent> context, IBehavior<OrderState, OrderSubmittedStateIntegrationEvent> next)
    {
        try
        {
            ...
            var response = await _requestCheckStockForOrderIntegrationCommand
                .GetResponse<JsonErrorResponse, CheckStockResponse>(checkStockForOrderIntegrationCommand);

            // Check Error in Catalog Microservice
