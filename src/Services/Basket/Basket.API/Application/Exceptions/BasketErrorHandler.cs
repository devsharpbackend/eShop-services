...
public class BasketErrorHandler
{
    ...
    public BasketErrorHandler(IErrorHandler errorHandler, IWebHostEnvironment env, ILogger<HttpGlobalExceptionFilter> logger)
    {
    ...
    public  JsonErrorResponse GetError(Exception Exception)
    {
        ...
        if (Exception.GetType() == typeof(BasketDomainException))
        {
            var problemDetails = new ValidationProblemDetails()
            {
