...
public class GetCatalogListQueryHandler : IRequestHandler<GetCatalogListQuery, CatalogListVM>
{
    ...
    public GetCatalogListQueryHandler(IOptionsSnapshot<CatalogSetting> options, IDiscountService discountService)
    {
    ...
    public async Task<CatalogListVM> Handle(GetCatalogListQuery request, CancellationToken cancellationToken)
    {
        using (var connection = new SqlConnection(options.Value.ConnectionString))
        {
            ...
            catalogListVM.Data= await connection.QueryAsync<CatalogListItemVM>(@"select cat.IsDiscount,cat.Discount, cat.Name,cat.Id,cat.Description,cat.Price,cat.AvailableStock,type.Type as CatalogTypeName
            ...
            //    {
            //        var discount=await _discountService.GetDiscounts(catalog.Id);
