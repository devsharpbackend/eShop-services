...
public  class CatalogErrorHandler
{
    ...
    public CatalogErrorHandler(IErrorHandler errorHandler, IWebHostEnvironment env, ILogger<HttpGlobalExceptionFilter> logger)
    {
    ...
    public  JsonErrorResponse GetError(Exception Exception)
    {
        ...
        if (Exception.GetType() == typeof(CatalogDomainException))
        {
            var problemDetails = new ValidationProblemDetails()
            {
