...
namespace eShop.Services.Catalog.Domain.AggregatesModel.CatalogAggregate;
...
public class CatalogItem : Entity, IAggregateRoot
{
    ...
    public CatalogItem(string? name, string? description, decimal price, bool isDiscount, string? pictureFileName, int catalogTypeId, int availableStock, int stockThreshold, int maxStockThreshold)
    {
        this._name = String.IsNullOrEmpty(name)? throw new CatalogDomainException("The name is empty and must be entered"):name;
        this._description = description;
        this._price = price<=0?throw new CatalogDomainException("The price must be than 0.") :price;
